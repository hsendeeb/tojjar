        ...
        <table class="table table-striped p-2">
            ...
            <tr>
                ...
                <td>{{ $record->user->phone }}</td>
                 ...
                 <td>{{ $record->status }}</td>
                  <td>{{ ($record->user->premium) ? 'Yes' : 'No'}}</td>
                <td>
                    <img class="rounded-circle" style="width:30px;height:30px" src='{{ Storage::url($record->user->image) }}'>
                </td>
                <td>
                  <a target="_blank" href=" {{ Storage::url($record->invoice_image)}}"> <img  class="rounded-circle" style="width:30px;height:30px" src='{{ Storage::url($record->invoice_image) }}'></a> 
                </td>
                <td>
                    ...
                    <form action="{{ route('admin.accept',['id'=>$record->id,'user_id'=>$record->user->id]) }}" method="POST">
                    @csrf
