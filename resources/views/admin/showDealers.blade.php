...
<body class="font-sans antialiased">
  ...
  <div class="table-responsive-sm mt-5 p-5">
    ...
    <table class="table table-striped">
      <tr>
        <th class=" text-center bg-warning archivo rounded-pill px-2 py-1">Records:{{count($dealers)}}</th>
      </tr>
      <tr>
        <th>Id</th>
        <th>name</th>
      ...
      @foreach ($dealers as $dealer)
        ...
        <tr>
          ...
          <td><a href="{{ route('profile.show', $dealer->id) }}">{{ $dealer->user->name }}</a></td>
          <td>{{ $dealer->user->last_name }}</td>
          <td>{{ $dealer->user->email }}</td>
          <td>{{ $dealer->user->phone }}</td>
          <td>{{ $dealer->user->status }}</td>
          <td><img class="rounded-circle" style="width:30px;height:30px" src='{{ Storage::url($dealer->user->image) }}'>
          </td>
          <td>
           
              <form action="{{ route('admin.delete', $dealer->id) }}" method="POST">
                @csrf
                @method('DELETE')
