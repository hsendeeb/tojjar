            ...
            @forelse ($vehicles as $vehicle)
                <div class="card mb-3 w-75 mx-auto p-0 mt-3 notHover">
                    <div class="row g-0">
                        ...
                        <div class="col-md-8">
                            <div class="card-body">
                                <h5 class="card-title">{{$vehicle->company->company_name}} <span
                                        class="brush px-3 py-1">{{$vehicle->model->model_name}}</span>
                                    <div
                            ...
                            <div class="card-footer mb-0 d-flex gap-2">
                                <form class="w-75" action="{{ route('vehicle.edit', $vehicle) }}">
                                    @csrf
                                    @method('PUT')
                                    <button class="btn btn-outline-primary w-100" type="submit">Edit</button>
                                </form>
                                <form action="{{ route('vehicle.destroy', $vehicle) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="btn btn-danger " type="submit"><span><i class="bi bi-trash-fill"></i>
                                        </span></button>
